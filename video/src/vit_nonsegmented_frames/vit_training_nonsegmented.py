...
def main():
    ...
    from load_dataset import get_frame_splits
    ...
    FRAME_SUBSAMPLE_EVERY = CFG['training']['frame_subsample_every']
    ...
    splits = get_frame_splits(
        image_size      = IMAGE_SIZE,
        train_transform = train_tf,
        eval_transform  = eval_tf,
        subsample_every  = FRAME_SUBSAMPLE_EVERY,
    )
    ...
    # Weighted loss: penalise minority class ("With A-H") errors more
    loss_weights = torch.tensor(class_weights / class_weights.sum(), dtype=torch.float32).to(DEVICE)
    criterion = nn.CrossEntropyLoss(weight=loss_weights)

    # Differential LRs: encoder gets 10× smaller LR than head
    LR_ENCODER = LR * 0.1
    ...
    best_f1    = -1.0
    no_improve = 0
    GRAD_CLIP  = 1.0   # max gradient norm — prevents encoder destabilization
    ...
    print(classification_report(all_labels, all_preds, target_names=['No A-H', 'With A-H'], digits=4))
