"""
ViViT (Video Vision Transformer) Training Pipeline
====================================================

Trains ``VivitForVideoClassification`` on sliding-window clips of frames
for binary A/H detection.

Clip-level labelling
--------------------
Since the dataset only has per-frame and per-video labels (no fragment-level
markup), we derive clip labels with the ``label_agg="any"`` strategy:
a clip is labelled **1 (With A/H)** if *at least one* of its constituent
frames carries label 1.

Usage
-----
    cd video/src
    python vivit_training.py
"""
...
def main():
    ...
    # NOTE: IMG_MEAN / IMG_STD are not used — VivitImageProcessor applies
    # its own Kinetics-400 normalisation stats automatically.

    VIVIT_NAME = CFG["vivit"]["model_name"]
    CLIP_LEN = CFG["vivit"]["clip_len"]
    CLIP_STRIDE = CFG["vivit"]["clip_stride"]
    LABEL_AGG = CFG["vivit"]["label_agg"]

    EPOCHS = CFG["vivit"]["training"]["epochs"]
    BATCH_SIZE = CFG["vivit"]["training"]["batch_size"]
    LR = CFG["vivit"]["training"]["learning_rate"]
    WD = CFG["vivit"]["training"]["weight_decay"]
    PATIENCE = CFG["vivit"]["training"]["early_stopping_patience"]

    WEIGHTS = (
        REPO_ROOT
        / "video"
        / CFG["vivit"]["output"]["weights_dir"]
        / CFG["vivit"]["output"]["weights_name"]
    )
    ...
    print("=" * 70)
    print("ViViT Training Pipeline — BAH A/H Detection")
    print("=" * 70)
    print(f"Device       : {DEVICE}")
    print(f"Model        : {VIVIT_NAME}")
    ...
    # ── VivitImageProcessor for normalisation ────────────────────────────────
    processor = VivitImageProcessor.from_pretrained(VIVIT_NAME)
    print(f"\n[Processor] VivitImageProcessor loaded from {VIVIT_NAME}")
    ...
    # We pass transform=None so __getitem__ returns raw uint8-normalised
    # tensors (0-1 float).  The VivitImageProcessor will handle the rest.
    ...
    for split_name in ("train", "val", "test"):
        ...
        # Count unique video folders
        folders = set(clip[0] for clip in ds.clips)
        # Check clip lengths (how many frames each clip actually has)
    ...
    print(f"  Raw clip tensor : shape={sample_clip.shape}, "
          f"dtype={sample_clip.dtype}, "
          f"min={sample_clip.min():.3f}, max={sample_clip.max():.3f}")
    print(f"  Label           : {sample_label}")
    # Check that frames are not all-zero (i.e. images were actually loaded)
    nonzero_frames = sum(1 for t in range(sample_clip.shape[0]) if sample_clip[t].abs().sum() > 0)
    ...
    # ── Collate function ─────────────────────────────────────────────────────
    def vivit_collate_fn(batch):
        """
        Collate clips into VivitImageProcessor format.

        Each item in batch is (clip_tensor, label) where clip_tensor has
        shape (clip_len, C, H, W) with values in [0, 1].

        VivitImageProcessor expects a list of "videos", each video being
    ...
    print(f"  pixel_values : shape={first_pv.shape}, dtype={first_pv.dtype}, "
    ...
    # ── Model ────────────────────────────────────────────────────────────────
    print("\n[Model] Loading VivitForVideoClassification ...")
    ...
    # ViViT has model.vivit.embeddings, model.vivit.encoder.layer[0..11],
    # model.classifier
    # Freeze embeddings and first 8 encoder layers; fine-tune last 4 + head
    n_encoder_layers = len(model.vivit.encoder.layer)
    FREEZE_LAYERS = 8
    print(f"[Model] Encoder has {n_encoder_layers} layers")
    print(f"[Model] Freezing: embeddings + first {FREEZE_LAYERS}/{n_encoder_layers} encoder layers")
    print(f"[Model] Trainable: last {n_encoder_layers - FREEZE_LAYERS} encoder layers + classifier head")
    ...
    PLOT_PATH = WEIGHTS.parent / "vivit_training_curves.png"
