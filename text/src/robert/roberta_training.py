"""
RoBERTa Text Classification Training Pipeline
===============================================

Fine-tunes ``RobertaForSequenceClassification`` on interview transcripts
...
def main():
    ...
    ROBERTA_NAME = CFG["roberta"]["model_name"]
    FREEZE_LAYERS = CFG["roberta"]["freeze_layers"]

    EPOCHS = CFG["roberta"]["training"]["epochs"]
    BATCH_SIZE = CFG["roberta"]["training"]["batch_size"]
    LR = CFG["roberta"]["training"]["learning_rate"]
    WD = CFG["roberta"]["training"]["weight_decay"]
    PATIENCE = CFG["roberta"]["training"]["early_stopping_patience"]
    ...
    WEIGHTS = (
        REPO_ROOT
        / "text"
        / CFG["roberta"]["output"]["weights_dir"]
        / CFG["roberta"]["output"]["weights_name"]
    )
    ...
    print("=" * 70)
    print("RoBERTa Training Pipeline — BAH A/H Text Classification")
    print("=" * 70)
    ...
    # ── Tokenizer ────────────────────────────────────────────────────────────
    print(f"\n[Tokenizer] Loading AutoTokenizer from {ROBERTA_NAME} ...")
    ...
    # ── Model ────────────────────────────────────────────────────────────────
    print(f"\n[Model] Loading RobertaForSequenceClassification from {ROBERTA_NAME} ...")
    ...
    # ── Freeze early layers ──────────────────────────────────────────────────
    # RoBERTa structure: model.roberta.embeddings, model.roberta.encoder.layer[0..11]
    n_encoder_layers = len(model.roberta.encoder.layer)
    print(f"[Model] RoBERTa encoder has {n_encoder_layers} layers")
    print(
    ...
    PLOT_PATH = WEIGHTS.parent / "roberta_training_curves.png"
    ...
    print(f"\n{'=' * 70}")
    print(f"Test Results — RoBERTa ({ROBERTA_NAME})")
    print(f"{'=' * 70}")
    ...
    # ── Summary ──────────────────────────────────────────────────────────────
    print(f"\n{'=' * 70}")
    print(f"RoBERTa Training Summary")
    print(f"{'=' * 70}")
    print(f"  Model           : {ROBERTA_NAME}")
