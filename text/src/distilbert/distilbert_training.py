"""
DistilBERT Text Classification Training Pipeline
==================================================

Fine-tunes ``DistilBertForSequenceClassification`` on interview transcripts
...
def main():
    ...
    DISTILBERT_NAME = CFG["distilbert"]["model_name"]
    FREEZE_LAYERS = CFG["distilbert"]["freeze_layers"]

    EPOCHS = CFG["distilbert"]["training"]["epochs"]
    BATCH_SIZE = CFG["distilbert"]["training"]["batch_size"]
    LR = CFG["distilbert"]["training"]["learning_rate"]
    WD = CFG["distilbert"]["training"]["weight_decay"]
    PATIENCE = CFG["distilbert"]["training"]["early_stopping_patience"]
    GRAD_CLIP = CFG["training"]["grad_clip"]
    SEED = CFG["training"]["seed"]
    LABEL_SMOOTH = float(CFG["distilbert"].get("label_smoothing", 0.0))

    WEIGHTS = (
        REPO_ROOT
        / "text"
        / CFG["distilbert"]["output"]["weights_dir"]
        / CFG["distilbert"]["output"]["weights_name"]
    )
    ...
    print("=" * 70)
    print("DistilBERT Training Pipeline — BAH A/H Text Classification")
    print("=" * 70)
    print(f"Device        : {DEVICE}")
    print(f"Model         : {DISTILBERT_NAME}")
    ...
    # ── Tokenizer ────────────────────────────────────────────────────────────
    print(f"\n[Tokenizer] Loading AutoTokenizer from {DISTILBERT_NAME} ...")
    ...
    # ── Model ────────────────────────────────────────────────────────────────
    print(f"\n[Model] Loading DistilBertForSequenceClassification from {DISTILBERT_NAME} ...")
    ...
    # ── Freeze early layers ──────────────────────────────────────────────────
    # DistilBERT structure: model.distilbert.embeddings,
    #                       model.distilbert.transformer.layer[0..5]
    n_encoder_layers = len(model.distilbert.transformer.layer)
    print(f"[Model] DistilBERT transformer has {n_encoder_layers} layers")
    print(
        f"[Model] Freezing: embeddings + first {FREEZE_LAYERS}/{n_encoder_layers} transformer layers"
    )
    print(
        f"[Model] Trainable: last {n_encoder_layers - FREEZE_LAYERS} transformer layers + "
        f"pre_classifier + classifier head"
    )
    ...
    print(f"  Num heads      : {model.config.n_heads}")
    ...
    PLOT_PATH = WEIGHTS.parent / "distilbert_training_curves.png"
    ...
    print(f"\n{'=' * 70}")
    print(f"Test Results — DistilBERT ({DISTILBERT_NAME})")
    print(f"{'=' * 70}")
    ...
    # ── Summary ──────────────────────────────────────────────────────────────
    print(f"\n{'=' * 70}")
    print(f"DistilBERT Training Summary")
    print(f"{'=' * 70}")
    print(f"  Model           : {DISTILBERT_NAME}")
