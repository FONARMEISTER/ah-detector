"""
Wav2Vec2 Audio Classification Training Pipeline
=================================================

Fine-tunes ``Wav2Vec2ForSequenceClassification`` (facebook/wav2vec2-base)
on pre-extracted audio files for binary A/H (Ambivalence/Hesitancy) detection.

Model choice rationale
----------------------
``wav2vec2-base`` (94 M params) is chosen over the larger ``wav2vec2-large``
(317 M) for the same reason DistilBERT outperformed BERT/RoBERTa on the text
modality: smaller models generalise better on small datasets and train faster.
The base model is pretrained on 960 h LibriSpeech and provides strong speech
representations without overfitting risk.

Architecture
------------
- CNN feature extractor (frozen) → raw waveform → 512-d frame features
- 12-layer Transformer encoder (partially frozen, fine-tune last 4 layers)
- Mean-pooled CLS representation → 2-class linear head

Each sample is a waveform padded/truncated to ``max_length_sec`` seconds.
Labels:
  - 0 : No A/H
  - 1 : With A/H
...
def main():
    ...
    MODEL_NAME = CFG["wav2vec2"]["model_name"]
    FREEZE_FEATURE_EXTRACTOR = CFG["wav2vec2"]["freeze_feature_extractor"]
    FREEZE_LAYERS = CFG["wav2vec2"]["freeze_layers"]

    EPOCHS = CFG["wav2vec2"]["training"]["epochs"]
    BATCH_SIZE = CFG["wav2vec2"]["training"]["batch_size"]
    LR = CFG["wav2vec2"]["training"]["learning_rate"]
    WD = CFG["wav2vec2"]["training"]["weight_decay"]
    PATIENCE = CFG["wav2vec2"]["training"]["early_stopping_patience"]
    ...
    WEIGHTS = (
        REPO_ROOT
        / "audio"
        / CFG["wav2vec2"]["output"]["weights_dir"]
        / CFG["wav2vec2"]["output"]["weights_name"]
    )
    ...
    print("=" * 70)
    print("Wav2Vec2 Training Pipeline — BAH A/H Audio Classification")
    print("=" * 70)
    print(f"Device                   : {DEVICE}")
    print(f"Model                    : {MODEL_NAME}")
    print(f"Sample rate              : {SAMPLE_RATE} Hz")
    print(f"Max audio length         : {MAX_LENGTH_SEC}s "
          f"({int(SAMPLE_RATE * MAX_LENGTH_SEC):,} samples)")
    print(f"Freeze CNN extractor     : {FREEZE_FEATURE_EXTRACTOR}")
    print(f"Freeze transformer layers: {FREEZE_LAYERS}/12")
    ...
    # ── Feature extractor ────────────────────────────────────────────────────
    print(f"\n[FeatureExtractor] Loading Wav2Vec2FeatureExtractor from {MODEL_NAME} ...")
    ...
    # ── Dataset ──────────────────────────────────────────────────────────────
    print("\n[Dataset] Loading audio splits ...")
    t0 = time.time()
    splits = get_audio_splits(
    ...
    for split_name in ("train", "val", "test"):
        ...
        labels = [d["label"] for d in ds.video_data]
        cnt = Counter(labels)
        n_total = len(labels)
        pct_pos = cnt.get(1, 0) / n_total * 100 if n_total > 0 else 0
        n_audio = sum(1 for d in ds.video_data if d["source"] == "audio_dir")
        n_video = sum(1 for d in ds.video_data if d["source"] == "video_dir")
        print(
    ...
    if isinstance(sample_encoded, dict):
        ...
        print(f"  Waveform shape  : {sample_encoded.shape}")
        print(f"  Waveform range  : [{sample_encoded.min():.3f}, {sample_encoded.max():.3f}]")

    # ── Collate function ─────────────────────────────────────────────────────
    def audio_collate_fn(batch):
        """Collate feature-extracted audio samples into batched tensors."""
    ...
    train_labels = [d["label"] for d in splits["train"].video_data]
    ...
    # ── Model ────────────────────────────────────────────────────────────────
    print(f"\n[Model] Loading Wav2Vec2ForSequenceClassification from {MODEL_NAME} ...")
    ...
    # The CNN feature extractor converts raw waveform → frame-level features.
    # It is always frozen: it is not designed for gradient-based fine-tuning
    # and freezing it dramatically reduces memory and compute cost.
    if FREEZE_FEATURE_EXTRACTOR:
        model.freeze_feature_extractor()
        print(f"[Model] CNN feature extractor frozen (wav2vec2.feature_extractor)")

    # ── Freeze early transformer layers ──────────────────────────────────────
    # Wav2Vec2 transformer: model.wav2vec2.encoder.layers[0..11]
    n_encoder_layers = len(model.wav2vec2.encoder.layers)
    print(f"[Model] Wav2Vec2 transformer has {n_encoder_layers} layers")
    print(
        f"[Model] Freezing: first {FREEZE_LAYERS}/{n_encoder_layers} transformer layers"
    )
    print(
        f"[Model] Trainable: last {n_encoder_layers - FREEZE_LAYERS} transformer layers + "
        f"projector + classifier head"
    )
    ...
    print(f"  Attn dropout     : {model.config.attention_dropout}")
    print(f"  Feat proj dropout: {model.config.feat_proj_dropout}")
    print(f"  Classifier proj  : {model.config.classifier_proj_size}")
    ...
    PLOT_PATH = WEIGHTS.parent / "wav2vec2_training_curves.png"
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # Metrics (sample-level — each audio clip is one sample)
    ...
    print(f"\n{'=' * 70}")
    print(f"Test Results — Wav2Vec2 ({MODEL_NAME})")
    print(f"{'=' * 70}")
    ...
    # ── Summary ──────────────────────────────────────────────────────────────
    print(f"\n{'=' * 70}")
    print(f"Wav2Vec2 Training Summary")
    print(f"{'=' * 70}")
    print(f"  Model                  : {MODEL_NAME}")
    print(f"  CNN extractor frozen   : {FREEZE_FEATURE_EXTRACTOR}")
    print(f"  Frozen transformer layers: {FREEZE_LAYERS}/{n_encoder_layers}")
