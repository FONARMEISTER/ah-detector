...
-----
    cd audio/src
    python audio_training.py                  # uses [audio].default_model
    python audio_training.py wav2vec2
    python audio_training.py wav2vec2emotional
"""
...
def main():
    ...
    SUPPORTED = ("wav2vec2", "wav2vec2emotional")
    if len(sys.argv) > 1:
        MODEL_KEY = sys.argv[1].strip().lower()
    else:
        MODEL_KEY = str(CFG["audio"].get("default_model", "wav2vec2emotional")).lower()
    ...
    print("=" * 70)
    print(f"Audio Training Pipeline — BAH A/H Classification  [{MODEL_KEY}]")
    print("=" * 70)
    ...
    print(f"Freeze transformer layers: {FREEZE_LAYERS}")
    ...
    print(f"[Model] Replacing original head → 2-class A/H head")
