...
- 24-layer HuBERT-Large Transformer encoder (first 20 frozen, last 4 trainable)
- Mean-pooled representation → projector → 2-class linear head

This script mirrors ``wav2vecemotional_training.py`` but uses
``HubertForSequenceClassification`` instead of
``Wav2Vec2ForSequenceClassification``.  The inner encoder module is
``model.hubert`` (vs ``model.wav2vec2``).
"""
...
def main():
    ...
    MODEL_NAME = CFG["hubert"]["model_name"]
    FREEZE_FEATURE_EXTRACTOR = CFG["hubert"]["freeze_feature_extractor"]
    FREEZE_LAYERS = CFG["hubert"]["freeze_layers"]

    EPOCHS = CFG["hubert"]["training"]["epochs"]
    BATCH_SIZE = CFG["hubert"]["training"]["batch_size"]
    LR = CFG["hubert"]["training"]["learning_rate"]
    WD = CFG["hubert"]["training"]["weight_decay"]
    PATIENCE = CFG["hubert"]["training"]["early_stopping_patience"]
    GRAD_CLIP = CFG["training"]["grad_clip"]
    SEED = CFG["training"]["seed"]
    LABEL_SMOOTH = float(CFG["hubert"].get("label_smoothing", 0.0))

    WEIGHTS = (
        REPO_ROOT
        / "audio"
        / CFG["hubert"]["output"]["weights_dir"]
        / CFG["hubert"]["output"]["weights_name"]
    )
    ...
    print("=" * 70)
    print("HuBERT Training Pipeline — BAH A/H Audio Classification")
    print("=" * 70)
    ...
    print(f"Freeze transformer layers: {FREEZE_LAYERS} (of 24 in hubert-large)")
    ...
    print(
        ...
        f"gain={AUG_CFG.get('audio_gain', 0.2)}  "
        f"snr_db={AUG_CFG.get('audio_snr_db', 25.0)}  "
        f"shift={AUG_CFG.get('audio_time_shift_sec', 0.5)}s  "
        f"masks={AUG_CFG.get('audio_num_time_masks', 2)}×"
        f"{AUG_CFG.get('audio_time_mask_frac', 0.05)}"
    )
    ...
    # ── Model ────────────────────────────────────────────────────────────────
    print(f"\n[Model] Loading HubertForSequenceClassification from {MODEL_NAME} ...")
    print(f"[Model] Replacing original emotion head → A/H head (2 classes)")
    ...
    if FREEZE_FEATURE_EXTRACTOR:
        ...
        print(f"[Model] CNN feature extractor frozen")

    # ── Freeze early transformer layers ──────────────────────────────────────
    n_encoder_layers = len(model.hubert.encoder.layers)
    print(f"[Model] HuBERT transformer has {n_encoder_layers} layers")
    print(
    ...
    PLOT_PATH = WEIGHTS.parent / "hubert_training_curves.png"
    ...
    print(f"\n{'=' * 70}")
    print(f"Test Results — HuBERT ({MODEL_NAME})")
    print(f"{'=' * 70}")
    ...
    # ── Summary ──────────────────────────────────────────────────────────────
    print(f"\n{'=' * 70}")
    print(f"HuBERT Training Summary")
    print(f"{'=' * 70}")
    print(f"  Model                    : {MODEL_NAME}")
