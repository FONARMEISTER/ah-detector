...
- 24-layer XLS-R Transformer encoder (first 20 frozen, last 4 trainable)
...
def main():
    ...
    MODEL_NAME = CFG["wav2vec2emotional"]["model_name"]
    FREEZE_FEATURE_EXTRACTOR = CFG["wav2vec2emotional"]["freeze_feature_extractor"]
    FREEZE_LAYERS = CFG["wav2vec2emotional"]["freeze_layers"]

    EPOCHS = CFG["wav2vec2emotional"]["training"]["epochs"]
    BATCH_SIZE = CFG["wav2vec2emotional"]["training"]["batch_size"]
    LR = CFG["wav2vec2emotional"]["training"]["learning_rate"]
    WD = CFG["wav2vec2emotional"]["training"]["weight_decay"]
    PATIENCE = CFG["wav2vec2emotional"]["training"]["early_stopping_patience"]
    GRAD_CLIP = CFG["training"]["grad_clip"]
    SEED = CFG["training"]["seed"]
    LABEL_SMOOTH = float(CFG["wav2vec2emotional"].get("label_smoothing", 0.0))

    WEIGHTS = (
        REPO_ROOT
        / "audio"
        / CFG["wav2vec2emotional"]["output"]["weights_dir"]
        / CFG["wav2vec2emotional"]["output"]["weights_name"]
    )
    ...
    print("=" * 70)
    print("Wav2Vec2-Emotional Training Pipeline — BAH A/H Audio Classification")
    print("=" * 70)
    ...
    print(f"Freeze transformer layers: {FREEZE_LAYERS} (of 24 in xlsr-large)")
    ...
    print(f"[Model] Replacing original emotion head (7 classes) → A/H head (2 classes)")
    ...
    print(f"[Model] XLS-R transformer has {n_encoder_layers} layers")
    print(
    ...
    PLOT_PATH = WEIGHTS.parent / "wav2vec2emotional_training_curves.png"
    ...
    print(f"\n{'=' * 70}")
    print(f"Test Results — Wav2Vec2-Emotional ({MODEL_NAME})")
    print(f"{'=' * 70}")
    ...
    # ── Summary ──────────────────────────────────────────────────────────────
    print(f"\n{'=' * 70}")
    print(f"Wav2Vec2-Emotional Training Summary")
    print(f"{'=' * 70}")
    print(f"  Model                    : {MODEL_NAME}")
